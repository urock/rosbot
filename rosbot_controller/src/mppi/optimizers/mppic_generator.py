...
class MPPICGenerator():
    ...
    def _get_params(self):
        ...
        self.w_std = rospy.get_param("~generator/w_std", 0.1)
        self.limit_v = rospy.get_param("~generator/limit_v", 0.5)
        self.limit_w = rospy.get_param("~generator/limit_w", 0.7)
    ...
    def _update_batch_of_seqs(self):
        ...
        self._batch_of_seqs[:, :, 2:3] = np.clip(
            self._batch_of_seqs[:, :, 2:3], -self.limit_v, self.limit_v
        )

        self._batch_of_seqs[:, :, 3:4] = np.clip(
            self._batch_of_seqs[:, :, 3:4], -self.limit_w, self.limit_w
        )

        self._update_velocities()

    def _generate_noises(self, time_steps):
        v_noises = np.random.normal(0.0, self.v_std, size=(self.batch_size, time_steps, 1))
        w_noises = np.random.normal(0.0, self.w_std, size=(self.batch_size, time_steps, 1))
